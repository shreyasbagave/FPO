...
import { Package, ShoppingCart, Truck, FileText, LogOut, Bell, AlertCircle, User, ChevronDown, Menu } from 'lucide-react';
...
const RetailerDashboard = ({ user, onLogout }) => {
  ...
  const tabs = [
    { id: 'products', name: 'Products & Rates', icon: Package },
    { id: 'purchases', name: 'Purchase Requests', icon: ShoppingCart },
    { id: 'dispatch', name: 'Dispatch Details', icon: Truck },
    { id: 'payments', name: 'Payments & Invoices', icon: FileText },
  ];
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      >
        <div className="py-2 sm:py-4 flex items-center justify-between min-w-0" style={{ overflow: 'visible' }}>
          <div className="flex items-center gap-1.5 sm:gap-2 md:gap-4 min-w-0 flex-1">
            ...
            <h1 className="text-sm sm:text-base md:text-xl lg:text-2xl font-bold text-gray-800 truncate">Market Linkage Partner Dashboard</h1>
          </div>
              ...
              {showProfileMenu && (
                ...
                >
                  <div className="px-4 py-3 border-b border-gray-200">
                    ...
                    <p className="text-xs text-gray-500 mt-1">Market Linkage Partner</p>
                  </div>
                  <div className="py-1">
                    <button
                      onClick={() => {
      ...
      <div className="flex relative">
        ...
        <main className="flex-1 p-3 sm:p-4 md:p-6 w-full">
          {activeTab === 'products' && (
            <ProductView user={user} onAlert={addAlert} />
          )}
          {activeTab === 'purchases' && (
            <PurchaseRequests user={user} onAlert={addAlert} />
          )}
          {activeTab === 'dispatch' && (
            <DispatchDetails user={user} onAlert={addAlert} />
          )}
          {activeTab === 'payments' && (
            <PaymentInvoices user={user} onAlert={addAlert} />
          )}
        </main>
      </div>
